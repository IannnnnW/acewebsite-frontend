...
export default function MentorProfile({ mentor }) {
  ...
  const facts = [
    { label: t('mentor.institution'), value: mentor.institution },
    { label: t('mentor.country'), value: mentor.country },
    { label: t('mentor.expertise'), value: loc(mentor.expertise, locale) },
  ].filter((f) => f.value)

  return (
    <>
      <IdmPageHeader
        eyebrow={t('nav.mentors')}
        title={mentor.name}
        subtitle={loc(mentor.expertise, locale)}
        breadcrumb={[
          { label: t('nav.training'), href: '/edctp-idm/training' },
          { label: t('nav.mentors'), href: '/edctp-idm/training/mentors' },
          { label: mentor.name, href: '#' },
        ]}
      />
        ...
        <div className="mx-auto max-w-5xl px-6 lg:px-8">
          <div className="flex flex-col md:flex-row gap-10">
            ...
            <div className="md:w-72 shrink-0">
              <div className="relative aspect-square rounded-2xl overflow-hidden bg-idblue-50 shadow-md">
                {mentor.photo?.url ? (
                  ...
                  <img src={mentor.photo.url} alt={mentor.name} className="w-full h-full object-cover" />
                ) : (
            ...
            <div className="flex-1">
              ...
              {loc(mentor.bio, locale) && (
                <p className="text-gray-700 leading-relaxed whitespace-pre-wrap">{loc(mentor.bio, locale)}</p>
              )}

              <Link
                href="/edctp-idm/training/mentors"
              ...
              >
                <ArrowLeftIcon className="h-4 w-4" />{t('mentor.backToMentors')}
              </Link>
            </div>
