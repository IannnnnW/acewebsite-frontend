...
function LanguageToggle() {
  ...
  return (
    <div className="flex items-center rounded-full border border-idblue-200 overflow-hidden text-xs font-semibold shrink-0">
      <button
        onClick={() => setLocale('en')}
        className={`px-3 py-1.5 transition-colors ${
          locale === 'en' ? 'bg-idblue-500 text-white' : 'text-idblue-700 hover:bg-idblue-50'
        }`}
      >
        EN
      </button>
      <button
        ...
        className={`px-3 py-1.5 transition-colors ${
          locale === 'fr' ? 'bg-idblue-500 text-white' : 'text-idblue-700 hover:bg-idblue-50'
        }`}
      >
        FR
      </button>
...
export default function EdctpHeaderNav({ logoUrl, programName }) {
  ...
  const linkClass = (href) =>
    `relative px-4 py-2 text-base font-medium transition-colors whitespace-nowrap ${
      isActive(href)
        ...
        : 'text-gray-700 hover:text-idblue-500'
    }`
  ...
  return (
    <header className="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm">
      <div className="mx-auto max-w-7xl px-6 py-1.5 flex items-center justify-between gap-4">
        <Link href="/edctp-idm" className="flex items-center shrink-0">
          {logoUrl ? (
            ...
            <span className="font-bold text-idblue-900 text-lg tracking-tight">{programName}</span>
          )}
        </Link>

        <button
          className="lg:hidden text-idblue-900 p-1.5 rounded-md hover:bg-idblue-50"
        ...
        <div className="hidden lg:flex items-center gap-4 shrink-0">
          <LanguageToggle />
          <Link href="/" className="inline-flex items-center gap-1 text-xs text-idblue-600 hover:text-idblue-800 transition-colors">
      ...
      {mobileOpen && (
        <nav className="lg:hidden border-t border-gray-200 flex flex-col px-6 py-3 gap-1 bg-white">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              onClick={() => setMobileOpen(false)}
              className={`py-2.5 text-base font-medium border-b border-gray-100 last:border-0 ${
                isActive(item.href) ? 'text-idblue-500 font-semibold' : 'text-gray-700 hover:text-idblue-500'
              }`}
            >
              {item.label}
            </Link>
          ...
          <div className="pt-3 flex items-center justify-between">
            <LanguageToggle />
            <Link href="/" className="inline-flex items-center gap-1 text-xs text-idblue-600 hover:text-idblue-800">
              <ArrowLeftIcon className="h-3 w-3" />{t('nav.backToAce')}
            </Link>
          </div>
        </nav>
