      ...
      <div className="space-y-16">
        {milestones.map((milestone, index) => {
          ...
          return (
            ...
            >
              {/* Content Card — image + text split when a milestone image is
                  set, so the card uses its full width instead of leaving the
                  other half of the row empty. Text-only when no image. */}
              <div
              ...
              >
                <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow border border-gray-100 group overflow-hidden">
                  <div className={hasImage ? 'grid grid-cols-1 sm:grid-cols-2' : ''}>
                    {hasImage && (
                      <div className={`relative h-48 sm:h-auto min-h-[200px] ${isEven ? 'sm:order-1' : 'sm:order-2'}`}>
                        <Image
                          src={urlFor(milestone.image).width(600).height(600).url()}
                    ...
                    <div className={`p-8 flex flex-col justify-center ${hasImage ? (isEven ? 'sm:order-2' : 'sm:order-1') : ''} ${isEven ? 'lg:text-right' : 'lg:text-left'} text-center lg:text-inherit`}>
                      {/* Year Badge */}
                      <div className={`mb-4 ${isEven && !hasImage ? 'lg:justify-end' : isEven ? '' : 'lg:justify-start'} flex justify-center`}>
